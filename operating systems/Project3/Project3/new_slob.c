...
static void *slob_page_alloc(struct page *sp, size_t size, int align)
{
	...
	int delta = 0, units = SLOB_UNITS(size);
		...
		/*if (calls == 6000) {
			printk("slob_alloc: Request: %u\nslob_alloc: Candidate blocks size:", size);
		}*/
		for (prev = NULL, cur = sp->freelist; ; prev = cur, cur = slob_next(cur)) {
			...
			if ((avail >= (units + delta)) && ((cur_best == NULL) || (avail - (units + delta) < best_fit))){
				// if (calls == 6000)
			...
			if (slob_last(cur)){
				if (cur_best != NULL){
					...
					if (calls >= 6000){ //Print that is required from clarification
						...
						if (slob_units(cur_best) >= units){
							printk("\nslob_alloc:Best Fit: %d", slob_units(cur_best));
						}else {
