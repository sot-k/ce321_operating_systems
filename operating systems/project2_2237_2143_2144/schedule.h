...
/* ---------------- Do NOT Touch END-------------- */
	...
	unsigned long long Latest_timestamp;//latest timestamp only changes on
//two ocassions 1) process losing cpu and 2) process joining running queue
//for a running process the latest timestamp is the time it is removed from the cpu
//for a process outside of running queue the latest timestamp is the time it joins the queue
	unsigned long long Starting_cpu_timestamp;
	unsigned long long WaitingInRQ;
